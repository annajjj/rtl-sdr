...
sf::Int16* raw = new sf::Int16[2*BUFF_SIZE];
...
sf::Int16* esl = new sf::Int16[2*SIZE*BWAUDIO / FS];
...
void LmRreconstruction() {
	...
	for (uint64_t i = 2, j = 0; i < SIZE*BWAUDIO / FS; i++) {
		...
		esl[2*i] = 10000 * (ym[i] + esrl[i]) / 2;
		esl[2*i+1] = 10000 * (ym[i] - esrl[i]) / 2;
	}
